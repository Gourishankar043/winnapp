...
class VisitRepositoryImpl implements VisitRepository {
  ...
  Future<List<Visit>> getVisits() async {
    ...
    final visits = [...localVisits, ...visitLog];
  ...
  Future<Visit> updateVisit(Visit visit) async {
    final model = VisitMapper.toModel(visit);
    final localRowsUpdated = await localDataSource.updateLocalVisit(model);
    if (localRowsUpdated > 0) return visit;
    final logRowsUpdated = await localDataSource.updateVisitLog(model);
    if (logRowsUpdated > 0) return visit;
    throw StateError(
      'Unable to update visit: no visit found with id ${visit.id}.',
    );
  }
  @override
