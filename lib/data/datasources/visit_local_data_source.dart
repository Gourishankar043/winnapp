...
class VisitLocalDataSourceImpl implements VisitLocalDataSource {
  ...
  Future<List<VisitModel>> getLocalVisits() async {
    ...
    final rows = await db.query(_localVisitsTable, orderBy: 'created_at DESC');
    return rows.map(_localRowToModel).toList();
  }
  @override
  Future<List<VisitModel>> getVisitLog() async {
    final db = await database;
    final rows = await db.query(_visitLogTable, orderBy: 'created_at DESC');
    return rows.map(_logRowToModel).toList();
  }
  @override
  Future<void> insertLocalVisit(VisitModel visit) async {
    final db = await database;
    await db.insert(_localVisitsTable, _modelToLocalRow(visit), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  @override
  Future<int> updateLocalVisit(VisitModel visit) async {
    final db = await database;
    return db.update(
      _localVisitsTable,
  ...
  Future<void> deleteLocalVisit(String id) async {
    final db = await database;
    await db.delete(_localVisitsTable, where: 'id = ?', whereArgs: [id]);
  }
  @override
  Future<void> insertVisitLog(VisitModel visit) async {
    final db = await database;
    await db.insert(_visitLogTable, _modelToLogRow(visit), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  @override
  Future<int> updateVisitLog(VisitModel visit) async {
    final db = await database;
    return db.update(
      _visitLogTable,
